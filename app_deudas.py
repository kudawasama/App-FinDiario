...
# Sección 2: Listar y Pagar
    ...
    if not df.empty:
        ...
        with st.form("form_registrar_pago"):
            deuda_a_pagar = st.selectbox("Selecciona la deuda a pagar:", options=df.index, format_func=lambda x: f"{df.loc[x, 'Acreedor']} - ${df.loc[x, 'Monto Restante']:.2f} restantes")
